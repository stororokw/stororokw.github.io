    ...
    // draw intersection h1
    {
        ...
        if(result.hit)
        {
...

            {
                ...
                drawText(new float2(p3.x + offset_wi.x * 8, p3.y  + offset_wi.y * 8), "ωi", "rgb(255 255 255)", undefined, "center");
                ...
                if(theta != 0)
                {
                    drawArcBetweenPoints(normalize(wi), normalize(wr), "white", viewProjection);
                    ...
                    drawText(angleTextPosition.add(new float2(0, 7)), "θ", "rgb(255 255 255)", "14px monospace", "center");

                    const midpoint1 = normalize(normalize(wr).add(normalize(result.normal)).mul(0.5));
                    ...
                    drawText(angleTextPosition1.add(new float2(0, 7)), "θ", "rgb(255 255 255)", "14px monospace", "center");
                }
                ...
                drawText(new float2(p9.x + 8, p9.y + 16), "hi", "rgb(255 255 255)", undefined, "left");
                ...
                const offset = normalize(p4.sub(p0));

                drawText(new float2(p4.x + offset.x * 16, p4.y + offset.y * 16), "ωr", "rgb(255 255 255)", undefined, "center");
                drawText(new float2(p7.x + 8, p7.y), "hr", "rgb(255 255 255)", undefined, "left");
                const lengthHR = ht_2.distance(wr);
                // drawText(new float2(p7.x + 8, p7.y + 16), `${lengthHR}`, "rgb(255 255 255)", undefined, "left");
                // drawText(new float2(p7.x + 8, p7.y + 32), `${Math.abs(Math.sin(theta))}`, "rgb(255 255 255)", undefined, "left");
                
                // project wr onto tangent axis
                const ht_3 = new float3(0, wr.y, 0);
                const axis = normalize(ht_3.sub(cylinder.center));
                drawArcBetweenPoints(normalize(wr), normalize(axis), "white", viewProjection);
                
                let a1 = Math.acos(normalize(wr).dot(normalize(axis)));
                // drawText(new float2(p7.x + 8, p7.y + 16), `${radToDeg(a1).toFixed(2)}`, "rgb(255 255 255)", undefined, "left");
                {
                    const midpoint = normalize(normalize(wr).add(normalize(axis)).mul(0.5));
                    const angleTextVector = normalize(midpoint.sub(cylinder.center));
                    const angleTextPositionWS = cylinder.center.add(angleTextVector.mul(0.15));
                    const angleTextPosition = projectPointToScreen(angleTextPositionWS, width, height, viewProjection);
                    drawText(angleTextPosition.add(new float2(0, 7)), "β", "rgb(255 255 255)", "14px monospace", "center");
                }
                const p8 = projectPointToScreen(ht_3, width, height, viewProjection);
                // drawCircle(p8);
            }
    ...
    // draw intersection h2
    {
        ...
        if(result.hit)
        {
...

            {
                ...
                const p3 = projectPointToScreen(wi, width, height, viewProjection);
                // drawLine2D(p0, p3, 2, "rgb(255 255 255)");
                drawLineArrow2D(p0, p3, 2, "rgb(255 255 255 / 50%)", [5, 8]);
                drawText(new float2(p3.x + 16, p3.y), "ωi'", "rgb(255 255 255)", undefined, "left");
        
                const p4 = projectPointToScreen(wr, width, height, viewProjection);
                // drawLine2D(p0, p4, 2, "rgb(255 0 0)");
                drawLineArrow2D(p0, p4, 2, "rgb(255 0 0  / 50%)", [5, 8]);
        
                const hi_1 = new float3(wi.x, 0, wi.z);
                const p5 = projectPointToScreen(hi_1, width, height, viewProjection);
                drawLine2D(p3, p5, 2, "rgb(255 255 255 / 50%)", [5, 8]);
                // drawText(new float2(p5.x + 8, p5.y + 16), "hi,2", "rgb(255 255 255)", undefined, "left");
                // drawCircle(p5, 5, "rgb(255 255 255)");

                drawText(p2.add(normalize(p2.sub(p0)).mul(16)), "n'", "rgb(50 120 255 / 50%)",  "bold 24px monospace", "center");
                drawLine2D(p1, p2, 2, "rgb(255 255 255 / 50%)");
                drawLineArrow2D(p0, p2, 3, "rgb(50 120 255 / 50%)");

                const ht_2 = new float3(wr.x, 0, wr.z);
                const midpoint_ht_2 = wr.add(ht_2).mul(0.5);
                const p6 = projectPointToScreen(ht_2, width, height, viewProjection);
                const p7 = projectPointToScreen(midpoint_ht_2, width, height, viewProjection);
                drawLine2D(p4, p6, 2, "rgb(255 0 0 / 50%)", [5, 8]);
                drawText(new float2(p4.x - 16, p4.y + 16), "ωr'", "rgb(255 255 255)", undefined, "center");
                // drawText(new float2(p6.x + 16, p6.y + 16), "hr'", "rgb(255 255 255)", undefined, "left");
                drawText(new float2(p7.x + 8, p7.y), "hr'", "rgb(255 255 255)", undefined, "left");
            }
