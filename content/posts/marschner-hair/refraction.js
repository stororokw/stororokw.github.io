...
const canvas = document.querySelector('.hair-refraction'); 
let width = canvas.width = Math.min(innerWidth, 300);
let height = canvas.height = Math.min(innerWidth, 300);

const ctx = canvas.getContext('2d');

const thetaPicker = document.querySelector('#hair-refraction-theta');
const iorPicker = document.querySelector('#hair-refraction-ior');

const output1 = document.querySelector('.hair-refraction-theta');
const output2 = document.querySelector('.hair-refraction-ior');
...
function draw()
{
    ...
    const lineSegments1 = [];
    const lineSegment5 = new LineSegment(new float2(-1, hairHeight), new float2(1, hairHeight));
    ...
    // intersections for a smooth cylinder
    {
        ...
        if(intersection !== null && intersection.hit)
        {
            ...
            drawLineArrow(new float3(intersection.position), new float3(intersection.position.add(Rt.mul(0.5))), 2, "red");
            const p4 = projectPointToScreen(new float3(intersection.position.add(Rt.mul(0.5))), width, height, viewProjection);
            drawText(p4.add(normalize(p4.sub(p)).mul(16)), "T", "red", "16px monospace", "center");
            const ray1 = new Ray(intersection.position.add(normalize(intersection.normal).mul(-0.01)), Rt.mul(100));

            ctx.setLineDash([5, 8]);
            ctx.strokeStyle = "rgb(0 255 0)";
            drawLine(new float3(intersection.position), new float3(intersection.position.add(intersection.normal.mul(-0.5))), camera);
            ctx.setLineDash([]);

            ctx.strokeStyle = "white";
            const angle1 = Math.acos(dot(Rt, new float2(0, -1)));
            ctx.beginPath();
            ctx.arc(p.x, p.y, 50, Math.PI/2 , Math.PI/2 - angle1, true);
            ctx.stroke();

            const c = new float3(intersection.position.add(Rt.mul(0.2)));
            const d = new float3(intersection.position.add(intersection.normal.mul(-0.2)));

            const midpoint1 = c.add(d).mul(0.5);
            const p_mid1 = projectPointToScreen(midpoint1, width, height, viewProjection);
            drawText(p_mid1, `β`, undefined, "16px monospace", "center");

            const r1 = projectPointToScreen(new float2(-1, 0, 0), width, height, viewProjection);
            const r2 = projectPointToScreen(new float2(1, 0, 0), width, height, viewProjection);
            const r3 = projectPointToScreen(new float2(1, 1, 0), width, height, viewProjection);
            const r4 = projectPointToScreen(new float2(-1, 1, 0), width, height, viewProjection);

            ctx.fillStyle = 'rgb(0 255 255 / 10%)';
            ctx.fillRect(r1.x, r1.y, Math.abs(r3.x - r1.x), Math.abs(r3.y - r1.y));

            let value = Math.asin(Math.sin(theta) * 1.0 / ior);
            drawText(new float2(16, 12), `sin(β) = sin(θ) * η1 / η2 = sin(${radToDeg(value).toFixed(0)}°)`, undefined, "12px monospace", "left");
            drawText(new float2(16, 24), `θ: ${radToDeg(theta).toFixed(0)}°`, undefined, "12px monospace", "left");
            drawText(new float2(16, 36), `β: ${radToDeg(angle1).toFixed(0)}°`, undefined, "12px monospace", "left");
            drawText(new float2(16, 48), `η1: 1`, undefined, "12px monospace", "left");
            drawText(new float2(16, 60), `η2: ${ior}`, undefined, "12px monospace", "left");

            drawText(new float2(width - 100, height / 4), `η1: 1.0(air)`, undefined, "14px monospace", "left");
            drawText(new float2(width - 100, height / 1.5), `η2: ${ior.toFixed(2)}`, undefined, "14px monospace", "left");
