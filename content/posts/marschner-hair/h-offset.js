...
const canvas = document.querySelector('.h-offset');
...
const thetaPicker = document.querySelector('#theta');
const testerPicker = document.querySelector('#tester');

const output = document.querySelector('.theta');
const output2 = document.querySelector('.tester');
...
function drawText(p, text, color = "white", font = "14px monospace")
{
    ctx.font = font;
    ctx.fillStyle = color;
...

    {
...

        {
            ...
            // draw reflection vector
            {
                ...
                drawText(new float2(p3.x + n0.x, p3.y + n0.y + (p3.y > height /2 ? 16 : -7)), "R(p=0)", undefined, "bold 14px monospace");
                ctx.restore();
                // draw angle text R
                {
